      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-16 md:mb-20">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#0068FF]/10 border border-[#0068FF]/20 mb-6">
            ...
            <span className="text-sm font-semibold text-[#0068FF] tracking-wide">Trusted by Families</span>
          </div>
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-[#2C3D4B] mb-5">
            Real families,
            <br />
            <span className="text-[#0068FF]">real results.</span>
          </h2>
        ...
        <div className="stats-grid grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {stats.map((stat, i) => (
            <div key={i} className="stat-item text-center p-6 bg-white rounded-2xl border border-[#F0F2F7] shadow-sm">
              <div className="text-3xl md:text-4xl font-extrabold text-[#0068FF] tracking-tight mb-1">{stat.value}</div>
              <div className="text-sm font-semibold text-[#2C3D4B] mb-1">{stat.label}</div>
              <div className="text-xs text-[#8695A3]">{stat.description}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {testimonials.map((testimonial, i) => (
            <div key={i} className="testimonial-card bg-white rounded-2xl p-7 border border-[#F0F2F7] shadow-sm hover:shadow-md transition-shadow duration-300">
              {/* Stars */}
              ...
              {/* Quote */}
              <p className="text-[#6A7780] text-sm leading-relaxed mb-5 italic">
                "{testimonial.text}"
              </p>

              {/* Author */}
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0068FF] to-[#0051E8] flex items-center justify-center text-white font-bold text-xs">
                  {testimonial.name.split(" ").map(n => n[0]).join("")}
                </div>
                <div>
                  <div className="text-sm font-bold text-[#2C3D4B]">{testimonial.name}</div>
                  <div className="text-xs text-[#8695A3]">{testimonial.role}</div>
                </div>
              </div>
            </div>
          ))}
